"""
Unified tabular regression benchmark pipeline - Beijing PM2.5 dataset.
...
Dataset:
- UCI Beijing PM2.5 dataset, id=381
- 43,824 hourly climate and air-quality records from Beijing
- Meteorological variables include temperature, dew point, pressure, wind direction, wind speed, snow, and rain
- Target: pm2.5 concentration
- This script predicts pm2.5 by default.
...
Outputs:
benchmark_outputs_beijing_pm25_381/
    results/
        model_results.csv
...
OUTPUT_DIR = Path("benchmark_outputs_beijing_pm25_381")
TEST_SIZE = 0.20
CV_FOLDS = 5

# This dataset has 43,824 rows. Gaussian Process Regression is cubic in the
# number of rows, so only that model is capped.
MAX_TRAIN_ROWS_BY_MODEL = {
    "Gaussian Process Regression": 2500,
}
...
def load_beijing_pm25_dataset() -> pd.DataFrame:
    """Load UCI Beijing PM2.5 dataset using ucimlrepo."""
    try:
...
def prepare_beijing_pm25_features(df: pd.DataFrame, target_col: str) -> Tuple[pd.DataFrame, np.ndarray, List[str], List[str]]:
    """Prepare X and y for UCI 381 climate regression."""
    df = df.copy()
    if target_col not in df.columns:
        raise ValueError(f"Target column {target_col} not found. Available columns: {list(df.columns)}")

    # Convert numeric-looking text columns while leaving wind direction categorical.
    ...
    # Drop target and row identifier from X.
    drop_cols = [c for c in [target_col, "No"] if c in df.columns]
...
def make_one_hot_encoder() -> OneHotEncoder:
    """Create a dense OneHotEncoder compatible with old and new sklearn versions."""
    try:
...
def fit_predict_one_model(
    ...
    max_rows = MAX_TRAIN_ROWS_BY_MODEL.get(model_name)
    if max_rows is not None and len(X_train_raw) > max_rows:
        sampled_idx = np.random.default_rng(RANDOM_STATE).choice(len(X_train_raw), size=max_rows, replace=False)
        X_train_used = X_train_raw.iloc[sampled_idx]
        y_train_used = y_train[sampled_idx]
    else:
    ...
    if model_name == "FT-Transformer":
        X_train = preprocessor.fit_transform(X_train_used)
    ...
    result = BenchmarkResult(
        ...
        domain="Climate and Environment / Air Quality",
        target=TARGET_NAME,
        model=model_name,
        n_train_used=len(X_train_used),
        mae=float(mae),
        rmse=float(rmse),
        r2=float(r2),
...
def run_benchmark() -> None:
    ...
    print("Dataset: UCI Beijing PM2.5, id=381")
    print(f"Target: {TARGET_COLUMN} = {TARGET_NAME}")
    print("Same fixed train/test split is used for every model.")
    print("=" * 80)

    print("\nLoading dataset...")
    df = load_beijing_pm25_dataset()
    print(f"Raw data shape: {df.shape}")

    X, y, numeric_cols, categorical_cols = prepare_beijing_pm25_features(df, TARGET_COLUMN)
