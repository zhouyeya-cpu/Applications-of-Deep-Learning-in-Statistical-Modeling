"""
Unified tabular regression benchmark pipeline - Yacht Hydrodynamics dataset.
...
Dataset:
- UCI Yacht Hydrodynamics dataset, id=243
- 308 samples, 6 numeric features
- Targets: Residuary resistance
- This script uses Residuary resistance.
...
RANDOM_STATE = 42
DATASET_ID = 243
LOCAL_DATA_FILE = "yacht_hydrodynamics.data"                   # Local data file would be used when online loading fails
COLUMN_NAMES = [
...
]                                                           # Column names for local files without header. Leave empty if the file already has headers.
DATASET_NAME = "Yacht Hydrodynamics"                        # Dataset name
DOMAIN = "Physics and Chemistry"                            # Dataset domain description for reporting
DROP_LIST = []                                              # Columns excluded from model features, usually IDs or non-predictive variables
TARGET_COLUMN = "Residuary resistance"                      # Main prediction target for this regression
TARGET_LIST = ["Residuary resistance"]                      # Target-related columns removed from input features to avoid leakage.
...
def load_dataset() -> pd.DataFrame:
    ...
    try:
        ...
        if COLUMN_NAMES:
            df = pd.read_csv(local_path, header=None, names=COLUMN_NAMES, sep=r"\s+")
        else:
