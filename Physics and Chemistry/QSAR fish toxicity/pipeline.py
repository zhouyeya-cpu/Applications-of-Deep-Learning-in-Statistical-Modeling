"""
Unified tabular regression benchmark pipeline - QSAR fish toxicity dataset.
...
Dataset:
- UCI QSAR fish toxicity dataset, id=504
- 908 samples, 6 numeric features
- Targets: LC50
- This script uses LC50.
...
RANDOM_STATE = 42
DATASET_ID = 504
LOCAL_DATA_FILE = "qsar_fish_toxicity.csv"                  # Local data file would be used when online loading fails
COLUMN_NAMES = [
    "CIC0",
    "SM1_Dz_Z",
    "GATS1i",
...
DATASET_NAME = "QSAR fish toxicity"                         # Dataset name
DOMAIN = "Physics and Chemistry / QSAR Toxicity Modeling"   # Dataset domain description for reporting
DROP_LIST = []                                              # Columns excluded from model features, usually IDs or non-predictive variables
TARGET_COLUMN = "LC50"                                      # Main prediction target for this regression
TARGET_LIST = ["LC50"]                                      # Target-related columns removed from input features to avoid leakage.
...
def load_dataset() -> pd.DataFrame:
    ...
    try:
        ...
        if COLUMN_NAMES:
            df = pd.read_csv(local_path, header=None, names=COLUMN_NAMES, delimiter= ";")
        else:
