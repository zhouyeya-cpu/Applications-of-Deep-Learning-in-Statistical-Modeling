"""
Unified tabular regression benchmark pipeline - Concrete Compressive Strength dataset.
...
Dataset:
- UCI Infrared Concrete Compressive Strength dataset, id=165
- 1030 samples, 8 numeric features
- Targets: Concrete compressive strength
- This script uses Concrete compressive strength.
...
RANDOM_STATE = 42
DATASET_ID = 165
LOCAL_DATA_FILE = "Concrete_Data.xls"                       # Local data file would be used when online loading fails
COLUMN_NAMES = []                                           # Column names for local files without header. Leave empty if the file already has headers.
DATASET_NAME = "Concrete Compressive Strength"              # Dataset name
DOMAIN = "Physics and Chemistry / Construction Materials"   # Dataset domain description for reporting
DROP_LIST = []                                              # Columns excluded from model features, usually IDs or non-predictive variables
TARGET_COLUMN = "Concrete compressive strength"             # Main prediction target for this regression
TARGET_LIST = ["Concrete compressive strength"]             # Target-related columns removed from input features to avoid leakage.
...
def load_dataset() -> pd.DataFrame:
    ...
    try:
        ...
        if COLUMN_NAMES:
            df = pd.read_excel(local_path, header=None, names=COLUMN_NAMES)
        else:
