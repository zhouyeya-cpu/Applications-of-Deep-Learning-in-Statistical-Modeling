"""
Unified tabular regression benchmark pipeline - Infrared Thermography Temperature dataset.
...
Dataset:
- UCI Infrared Thermography Temperature dataset, id=925
- 1020 samples, 30 numeric features
- Targets: aveOralF, aveOralM
- This script uses aveOralM.
...
RANDOM_STATE = 42
DATASET_ID = 925
LOCAL_DATA_FILE = "FLIR_groups1and2.csv"                    # Local data file would be used when online loading fails
COLUMN_NAMES = []                                           # Column names for local files without header. Leave empty if the file already has headers.
DATASET_NAME = "Infrared Thermography Temperature"          # Dataset name
DOMAIN = "Health and Medicine"                              # Dataset domain description for reporting
DROP_LIST = ["SubjectID", "Cosmetics", "Time", "Date"]                   # Columns excluded from model features, usually IDs or non-predictive variables
TARGET_COLUMN = "aveOralM"                                  # Main prediction target for this regression
TARGET_LIST = ["aveOralF", "aveOralM"]                      # Target-related columns removed from input features to avoid leakage.
TARGET_NAME = TARGET_COLUMN                                 # Display name for the target variable
OUTPUT_DIR = Path(".")                                      # Directory where result tables and figures will be saved
TEST_SIZE = 0.20
CV_FOLDS = 5
...
def load_local_csv(local_path: Path) -> pd.DataFrame:
    """Load FLIR csv files whose first rows are not real headers."""
    raw = pd.read_csv(local_path, header=None, dtype=str, keep_default_na=False)
    header_rows = raw.index[
        raw.apply(
            lambda row: row.astype(str).str.strip().eq(TARGET_COLUMN).any(),
            axis=1
        )
    ]

    if len(header_rows) == 0:
        raise ValueError("Could not find header row containing" + TARGET_COLUMN + ".")
    ...
    # drop round 2, 3, and 4
    drop_round_234_cols = [
        c for c in df.columns
        if str(c).strip()[-1] in ["2", "3", "4"]
    ]
    df = df.drop(columns=drop_round_234_cols)
...
def load_dataset() -> pd.DataFrame:
    ...
    # if failed, load local data file
    try:
        ...
        print("Failed to load dataset from ucimlrepo.")
        print(f"Reason: {exc}")
        print("Trying to load local data file instead...")

        local_path = Path(LOCAL_DATA_FILE)

        if not local_path.exists():
            raise RuntimeError(
                f"Local data file not found: {LOCAL_DATA_FILE}\n"
                "Please put data file in the same folder as pipeline.py,\n"
                "or change LOCAL_DATA_FILE to the correct file name."
            ) from exc
    ...
    print(f"Loaded local dataset from: {local_path}")
    return df

def prepare_features(df: pd.DataFrame, target_col: str) -> Tuple[pd.DataFrame, np.ndarray, List[str]]:
    """Prepare X and y for regression."""
    df = df.copy()
    if target_col not in df.columns:
        raise ValueError(f"Target column {target_col} not found. Available columns: {list(df.columns)}")

    # Drop targets and non-predictive variables.
    drop_cols = [c for c in TARGET_LIST + DROP_LIST if c in df.columns]
...
def plot_results(
    ...
    ax.plot(y_true[sorted_idx], linewidth=2, label="True", marker="o", markersize=3)
    ...
    for model_name in shown_models:
        ax.plot(predictions[model_name][sorted_idx], linewidth=1.5, alpha=0.7, label=model_name)
...
def run_benchmark() -> None:
    ...
    print("Dataset: UCI {}, id={}".format(DATASET_NAME, DATASET_ID))
    ...
    X, y, numeric_cols = prepare_features(df, TARGET_COLUMN)
