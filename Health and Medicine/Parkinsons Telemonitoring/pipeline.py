"""
Unified tabular regression benchmark pipeline - Parkinsons Telemonitoring dataset.
...
Dataset:
- UCI Parkinsons Telemonitoring dataset, id=189
- 5875 samples, 19 numeric features
- Targets: total_UPDRS, motor_UPDRS
- This script uses total_UPDRS.
...
RANDOM_STATE = 42
DATASET_ID = 189
LOCAL_DATA_FILE = "parkinsons_updrs.data"                   # Local data file would be used when online loading fails
COLUMN_NAMES = []                                           # Column names for local files without header. Leave empty if the file already has headers.
DATASET_NAME = "Parkinsons Telemonitoring"                  # Dataset name
DOMAIN = "Health and Medicine / Remote Patient Monitoring"  # Dataset domain description for reporting
DROP_LIST = ["subject#"]                                    # Columns excluded from model features, usually IDs or non-predictive variables
TARGET_COLUMN = "total_UPDRS"                               # Main prediction target for this regression
TARGET_LIST = ["total_UPDRS", "motor_UPDRS"]                # Target-related columns removed from input features to avoid leakage.
